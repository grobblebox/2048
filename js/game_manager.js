...
GameManager.prototype.addStartTiles = function () {
  for (var i = 0; i < this.startTiles; i++) {
    if (this.turnnum % this.spawnfreq == 0) {
        this.addRandomTile();
    }
    this.turnnum++;
  }
...
GameManager.prototype.move = function (direction) {
    ...
    traversals.y.forEach(function (y) {
      ...
      if (tile) {
        ...
        if (next && Math.abs(next.value) === Math.abs(tile.value) && !next.mergedFrom) {
          ...
          // Update the score
          self.score += Math.abs(next.value) + Math.abs(tile.value);
        } 
        else {
  ...
  if (moved) {
    ...
    document.getElementById("nextup").innerText = this.turnnum % this.spawnfreq;
    this.turnnum += 1;
